...
import { CreateCamp, InterCampBack, UpdateCamp, UpdateBaan, Group, MyMap, InterPartBack, Size, Id } from "../models/interface"
import { calculate, conPartBackToFront, ifIsTrue, jsonToMapSize, removeDuplicate, resOk, sendRes, sizeJsonMod, sizeMapToJson, startJsonSize, startSize, stringToId, swop } from "./setup"
...
export async function addBaanRaw(camp: InterCampBack, name: string,
    groupRef: 'A' | 'B' | 'C' | 'Dog' | 'E' | 'F' | 'G' | 'H' | 'J' | 'K' | 'L' | 'M' | 'N' | 'P' | 'Q' | 'R' | 'S' | 'T' | 'null'): Promise<Id> {
...
async function addPartRaw(campId: Id, nameId: Id, isAuth: boolean): Promise<InterPartBack | null> {
    const camp = await Camp.findById(campId)
    const partNameContainer = await PartNameContainer.findById(nameId)
    if (!camp || !partNameContainer) {
        return null
    }
    const part = await Part.create({ campId: camp._id, nameId, partName: `${partNameContainer.name} ${camp.campName}`, isAuth })
...
export async function updateBaanRaw(update: UpdateBaan) {
    try {
        const { name, fullName, baanId, link, girlSleepPlaceId, boySleepPlaceId, normalPlaceId, nongSendMessage } = update
        const baan = await Baan.findById(baanId)
        if (!baan) {
            return false
        }
...
export async function createCamp(req: express.Request, res: express.Response) {
    try {
        ...
        let partNameContainerCoop = await PartNameContainer.findOne({ name: 'ประสาน' })
        if (!partNameContainerCoop) {
            partNameContainerCoop = await PartNameContainer.create({ name: 'ประสาน' })
        }
        let partNameContainerRegis = await PartNameContainer.findOne({ name: 'ทะเบียน' })
        if (!partNameContainerRegis) {
            partNameContainerRegis = await PartNameContainer.create({ name: 'ทะเบียน' })
        }
        let partNameContainerPeeBaan = await PartNameContainer.findOne({ name: 'พี่บ้าน' })
        if (!partNameContainerPeeBaan) {
            partNameContainerPeeBaan = await PartNameContainer.create({ name: 'พี่บ้าน' })
        }
        let partNameContainerWelfare = await PartNameContainer.findOne({ name: 'สวัสดิการ' })
        if (!partNameContainerWelfare) {
            partNameContainerWelfare = await PartNameContainer.create({ name: 'สวัสดิการ' })
        }
        let partNameContainerMed = await PartNameContainer.findOne({ name: 'พยาบาล' })
        if (!partNameContainerMed) {
            partNameContainerMed = await PartNameContainer.create({ name: 'พยาบาล' })
        }
        let partNameContainerPlan = await PartNameContainer.findOne({ name: 'แผน' })
        if (!partNameContainerPlan) {
            partNameContainerPlan = await PartNameContainer.create({ name: 'แผน' })
        }
        let partNameContainerPrStudio = await PartNameContainer.findOne({ name: 'PR/studio' })
        ...
        const peeBaan = await addPartRaw(camp._id, partNameContainerPeeBaan._id, false)
        ...
        if (memberStructure == 'nong->highSchool,pee->1year,peto->2upYear') {
            ...
            await camp.updateOne({ peePassIds: camp.peePassIds, baanBoardId: baan._id })
...
export async function forceDeleteCamp(req: express.Request, res: express.Response) {
...
async function forceDeleteCampRaw(campId: Id, res: express.Response | null) {
    try {
        ...
        while (i < camp.peeCampMemberCardIds.length) {
            ...
            await user.updateOne({ campMemberCardIds: swop(campMemberCard._id, null, user.campMemberCardIds), filterIds: swop(camp._id, null, user.filterIds), registerIds: swop(camp._id, null, user.registerIds) })
            await campMemberCard.deleteOne()
        }
        i = 0
        while (i < camp.nongCampMemberCardIds.length) {
            const campMemberCard = await CampMemberCard.findById(camp.nongCampMemberCardIds[i++])
            const user = await User.findById(campMemberCard?.userId)
            if (!user || !campMemberCard) {
                continue
            }
            await user.updateOne({ campMemberCardIds: swop(campMemberCard._id, null, user.campMemberCardIds) })
            let j = 0
        ...
        while (i < camp.petoCampMemberCardIds.length) {
            const campMemberCard = await CampMemberCard.findById(camp.petoCampMemberCardIds[i++])
            const user = await User.findById(campMemberCard?.userId)
            if (!user || !campMemberCard) {
                continue
            }
        ...
        if (camp.dataLock) {
            while (i < camp.nongHeathIssueIds.length) {
                const heathIssue = await HeathIssue.findById(camp.nongHeathIssueIds[i++])
                if (!heathIssue) {
                    continue
                }
                await heathIssue.updateOne({ campIds: swop(camp._id, null, heathIssue.campIds) })
            }
        } else {
            while (i < camp.nongCampMemberCardHaveHeathIssueIds.length) {
                ...
                await heathIssue.updateOne({ campMemberCardIds: swop(campMemberCard._id, null, heathIssue.campMemberCardIds) })
            }
        }
        i = 0
        if (camp.peeDataLock) {
            while (i < camp.peeHeathIssueIds.length) {
                const heathIssue = await HeathIssue.findById(camp.peeHeathIssueIds[i++])
                if (!heathIssue) {
                    continue
                }
                await heathIssue.updateOne({ campIds: swop(camp._id, null, heathIssue.campIds) })
        ...
        if (camp.petoDataLock) {
            while (i < camp.petoHeathIssueIds.length) {
                const heathIssue = await HeathIssue.findById(camp.petoHeathIssueIds[i++])
                if (!heathIssue) {
                    continue
                }
                await heathIssue.updateOne({ campIds: swop(camp._id, null, heathIssue.campIds) })
        ...
        while (i < camp.nongAnswerPackIds.length) {
            const answerPack = await AnswerContainer.findById(camp.nongAnswerPackIds[i++])
            if (!answerPack) {
                continue
            }
            ...
            await user.updateOne({ nongAnswerPackIds: swop(answerPack._id, null, user.nongAnswerPackIds) })
            await answerPack.deleteOne()
        }
        i = 0
        while (i < camp.peeAnswerPackIds.length) {
            const answerPack = await AnswerContainer.findById(camp.peeAnswerPackIds[i++])
            if (!answerPack) {
                continue
            }
            const user = await User.findById(answerPack.userId)
            ...
            await user.updateOne({ peeAnswerPackIds: swop(answerPack._id, null, user.peeAnswerPackIds) })
            await answerPack.deleteOne()
        }
        i = 0
        while (i < camp.choiceQuestionIds.length) {
            const choiceQuestion = await ChoiceQuestion.findById(camp.choiceQuestionIds[i++])
            if (!choiceQuestion) {
                continue
            }
            let j = 0
            while (j < choiceQuestion.answerIds.length) {
                ChoiceAnswer.findByIdAndDelete(choiceQuestion.answerIds[j++])
            }
        }
        i = 0
        while (i < camp.textQuestionIds.length) {
            const textQuestion = await TextQuestion.findById(camp.textQuestionIds[i++])
            if (!textQuestion) {
                continue
            }
            let j = 0
            while (j < textQuestion.answerIds.length) {
                ChoiceAnswer.findByIdAndDelete(textQuestion.answerIds[j++])
            }
        }
...
export async function saveDeleteCamp(req: express.Request, res: express.Response) {
...
export async function saveDeleteCampName(req: express.Request, res: express.Response) {
    try {
...
export async function forceDeleteCampName(req: express.Request, res: express.Response) {
...
export async function forceDeleteBaan(req: express.Request, res: express.Response) {
    ...
    let nongCampMemberCardHaveHeathIssueIds = camp.nongCampMemberCardHaveHeathIssueIds
    let peeCampMemberCardHaveHeathIssueIds = camp.peeCampMemberCardHaveHeathIssueIds
    let i = 0
    if (camp.dataLock) {
        while (i < baan.nongCampMemberCardHaveHeathIssueIds.length) {
            const campMemberCard = await CampMemberCard.findById(baan.nongCampMemberCardHaveHeathIssueIds[i++])
            if (!campMemberCard) {
                continue
            }
            const heathIssue = await HeathIssue.findById(campMemberCard.healthIssueId)
            ...
            await heathIssue.updateOne({ campIds: swop(camp._id, null, heathIssue.campIds) })
            nongCampMemberCardHaveHeathIssueIds = swop(campMemberCard._id, null, nongCampMemberCardHaveHeathIssueIds)
        }
    } else {
        while (i < baan.nongCampMemberCardHaveHeathIssueIds.length) {
            const campMemberCard = await CampMemberCard.findById(baan.nongCampMemberCardHaveHeathIssueIds[i++])
    ...
    while (i < baan.peeCampMemberCardHaveHeathIssueIds.length) {
        const campMemberCard = await CampMemberCard.findById(baan.peeCampMemberCardHaveHeathIssueIds[i++])
        if (!campMemberCard) {
            continue
        }
        ...
        peeCampMemberCardHaveHeathIssueIds = swop(campMemberCard._id, null, peeCampMemberCardHaveHeathIssueIds)
    }
    i = 0
    ...
    while (i < baan.nongCampMemberCardIds.length) {
        ...
        nongCampMemberCardIds = swop(campMemberCard._id, null, nongCampMemberCardIds)
        let j = 0
        ...
        while (j < campMemberCard.allChatIds.length) {
            ...
            await chat.updateOne({ campMemberCardIds: swop(campMemberCard._id, null, chat.campMemberCardIds) })
        }
        j = 0
    ...
    while (i < baan.peeCampMemberCardIds.length) {
        ...
        part.peeShirtSize.set(campMemberCard.size, calculate(part.peeShirtSize.get(campMemberCard.size), 0, 1))
        await part.updateOne({ peeCampMemberCardIds: swop(campMemberCard._id, null, part.peeCampMemberCardIds), peeShirtSize: part.peeShirtSize })
        let j = 0
        while (j < campMemberCard.allChatIds.length) {
            const chat = await Chat.findById(campMemberCard.allChatIds[j++])
            if (!chat) {
...
export async function saveDeleteBaan(req: express.Request, res: express.Response) {
    const baan = await Baan.findById(req.params.id)
    if (!baan) {
        sendRes(res, false)
        return
    ...
    await camp.updateOne({ nongModelIds: swop(baan.nongModelId as Id, null, camp.nongModelIds), peeModelIds })
    await NongCamp.findByIdAndDelete(baan.nongModelId)
    await CampStyle.findByIdAndDelete(baan.styleId)
    await baan.deleteOne()
    res.status(200).json({ success: true })
}
export async function saveDeletePart(req: express.Request, res: express.Response) {
    ...
    if (!user || (user.role != 'admin' && !user.authPartIds.includes(camp.partBoardId as Id)) || part.isAuth || part._id.equals(camp.partPeeBaanId)) {
        return res.status(403).json({ success: false })
    }
    ...
    camp.updateOne({ petoModelIds: swop(part.petoModelId as Id, null, camp.petoModelIds) })
    await PetoCamp.findByIdAndDelete(part?.petoModelId)
    part.deleteOne()
    res.status(200).json({ success: true })
}
export async function forceDeletePart(req: express.Request, res: express.Response) {
...
async function forceDeletePartRaw(partId: Id) {
    ...
    while (i < part.peeCampMemberCardHaveHeathIssueIds.length) {
        const campMemberCard = await CampMemberCard.findById(part.peeCampMemberCardHaveHeathIssueIds[i++])
        if (!campMemberCard) {
            continue
        }
        const peeCamp = await PeeCamp.findById(campMemberCard.campModelId)
    ...
    if (camp.petoDataLock) {
        while (i < part.petoCampMemberCardHaveHeathIssueIds.length) {
            const campMemberCard = await CampMemberCard.findById(part.petoCampMemberCardHaveHeathIssueIds[i++])
            if (!campMemberCard) {
                continue
            }
            const heathIssue = await HeathIssue.findById(campMemberCard.healthIssueId)
            ...
            await heathIssue.updateOne({ campIds: swop(camp._id, null, heathIssue.campIds) })
            petoCampMemberCardHaveHeathIssueIds = swop(campMemberCard._id, null, petoCampMemberCardHaveHeathIssueIds)
        }
    } else {
        while (i < part.petoCampMemberCardHaveHeathIssueIds.length) {
            const campMemberCard = await CampMemberCard.findById(part.petoCampMemberCardHaveHeathIssueIds[i++])
    ...
    await revalidationHeathIssues(part.peeHeathIssueIds)
    ...
    while (i < part.petoCampMemberCardIds.length) {
        ...
        petoCampMemberCardIds = swop(campMemberCard._id, null, petoCampMemberCardIds)
        if (campMemberCard.sleepAtCamp) {
...
export async function saveDeletePartName(req: express.Request, res: express.Response) {
    try {
...
export async function forceDeletePartName(req: express.Request, res: express.Response) {
...
export async function addMoreBoard(req: express.Request, res: express.Response) {
...
export async function getCampNames(req: express.Request, res: express.Response) {
    const nameContainers = await NameContainer.find()
    res.status(200).json(nameContainers)
}
export async function createBaanByGroup(req: express.Request, res: express.Response) {
...
export async function getPartNames(req: express.Request, res: express.Response) {
...
export async function addAllGroup(req: express.Request, res: express.Response) {
    ...
    if (camp.ready.includes(baan.groupRef) || (!user.authPartIds.includes(camp.partCoopId as Id)) && !user.authPartIds.includes(camp.partBoardId as Id)) {
        sendRes(res, false)
        return
    }
    const { ready } = camp
...
export async function getAllRemainPartName(req: express.Request, res: express.Response) {
    const camp = await Camp.findById(req.params.id)
    if (!camp) {
        sendRes(res, false)
        return
...
export async function afterVisnuToPee(req: express.Request, res: express.Response) {
...
async function lockDataNong(campId: Id) {
    ...
    while (i < camp.nongCampMemberCardHaveHeathIssueIds.length) {
        ...
        await heathIssue.updateOne({
            ...
            campMemberCardIds: swop(campMemberCard._id, null, heathIssue.campMemberCardIds),
        })
    }
}
...
async function unlockDataNong(campId: Id) {
    ...
    while (i < camp1.nongCampMemberCardHaveHeathIssueIds.length) {
        const campMemberCard = await CampMemberCard.findById(camp1.nongCampMemberCardHaveHeathIssueIds[i++])
        if (!campMemberCard) {
            continue
        }
        const heathIssue = await HeathIssue.findById(campMemberCard.healthIssueId)
    ...
    while (i < camp2.nongModelIds.length) {
        ...
        const baanNongShirtSize = sizeMapToJson(baan.nongShirtSize as Map<Size, number>)
        const baanNongHaveBottleIds = baan.nongHaveBottleIds
        const baanNongHeathIssueIds = baan.nongHeathIssueIds
        const baanNongCampMemberCardHaveHeathIssueIds = baan.nongCampMemberCardHaveHeathIssueIds
        while (j < nongCamp.nongCampMemberCardIds.length) {
            const campMemberCard = await CampMemberCard.findById(nongCamp.nongCampMemberCardIds[j++])
            if (!campMemberCard) {
                continue
            }
            const user = await User.findById(campMemberCard.userId)
        ...
        await baan.updateOne({
            ...
            nongCampMemberCardHaveHeathIssueIds: baanNongCampMemberCardHaveHeathIssueIds,
    ...
    await camp2.updateOne({
        ...
        nongCampMemberCardHaveHeathIssueIds: campNongCampMemberCardHaveHeathIssueIds,
...
async function unlockDataPee(campId: Id) {
    ...
    while (i < camp1.peeCampMemberCardHaveHeathIssueIds.length) {
        const campMemberCard = await CampMemberCard.findById(camp1.peeCampMemberCardHaveHeathIssueIds[i++])
        if (!campMemberCard) {
            continue
        }
        const heathIssue = await HeathIssue.findById(campMemberCard.healthIssueId)
    ...
    while (i < camp2.peeModelIds.length) {
        ...
        const baanPeeShirtSize = sizeMapToJson(baan.peeShirtSize as Map<Size, number>)
        const baanPeeHaveBottleIds = baan.peeHaveBottleIds
        const baanPeeHeathIssueIds = baan.peeHeathIssueIds
        const baanPeeCampMemberCardHaveHeathIssueIds = baan.peeCampMemberCardHaveHeathIssueIds
        const partPeeSleepIds = part.peeSleepIds
        const partPeeShirtSize = sizeMapToJson(part.peeShirtSize as Map<Size, number>)
        const partPeeHaveBottleIds = part.peeHaveBottleIds
        const partPeeHeathIssueIds = part.peeHeathIssueIds
        const partPeeCampMemberCardHaveHeathIssueIds = part.peeCampMemberCardHaveHeathIssueIds
        while (j < peeCamp.peeCampMemberCardIds.length) {
            const campMemberCard = await CampMemberCard.findById(peeCamp.peeCampMemberCardIds[j++])
            if (!campMemberCard) {
                continue
            }
            const user = await User.findById(campMemberCard.userId)
        ...
        await baan.updateOne({
            ...
            peeCampMemberCardHaveHeathIssueIds: baanPeeCampMemberCardHaveHeathIssueIds,
        ...
        await part.updateOne({
            ...
            peeCampMemberCardHaveHeathIssueIds: partPeeCampMemberCardHaveHeathIssueIds,
...
async function unlockDataPeto(campId: Id) {
    ...
    while (i < camp1.petoCampMemberCardHaveHeathIssueIds.length) {
        const campMemberCard = await CampMemberCard.findById(camp1.petoCampMemberCardHaveHeathIssueIds[i++])
        if (!campMemberCard) {
            continue
        }
        const heathIssue = await HeathIssue.findById(campMemberCard.healthIssueId)
    ...
    while (i < camp2.petoModelIds.length) {
        ...
        const partPetoShirtSize = sizeMapToJson(part.petoShirtSize as Map<Size, number>)
        const partPetoHaveBottleIds = part.petoHaveBottleIds
        const partPetoHeathIssueIds = part.petoHeathIssueIds
        const partPetoCampMemberCardHaveHeathIssueIds = part.petoCampMemberCardHaveHeathIssueIds
        while (j < petoCamp.petoCampMemberCardIds.length) {
            const campMemberCard = await CampMemberCard.findById(petoCamp.petoCampMemberCardIds[j++])
            if (!campMemberCard) {
                continue
            }
            const user = await User.findById(campMemberCard.userId)
        ...
        await part.updateOne({
            ...
            petoCampMemberCardHaveHeathIssueIds: partPetoCampMemberCardHaveHeathIssueIds,
    ...
    await camp2.updateOne({
        ...
        petoCampMemberCardHaveHeathIssueIds: campPetoCampMemberCardHaveHeathIssueIds,
...
async function removeAnswer(userId: Id, campId: Id) {
    ...
    const answerContainer = await AnswerContainer.findById(camp.mapAnswerPackIdByUserId.get(user._id.toString()))
    if (!answerContainer) {
        return
    }
    ...
    while (i < answerContainer.choiceAnswerIds.length) {
        const choiceAnswer = await ChoiceAnswer.findById(answerContainer.choiceAnswerIds[i++])
        if (!choiceAnswer) {
            continue
        }
        const choiceQuestion = await ChoiceQuestion.findById(choiceAnswer.questionId)
        if (!choiceQuestion) {
            continue
        }
    ...
    while (i < answerContainer.textAnswerIds.length) {
        const textAnswer = await TextAnswer.findById(answerContainer.textAnswerIds[i++])
        if (!textAnswer) {
            continue
        }
        ...
        await textQuestion.updateOne({ answerIds: swop(textAnswer._id, null, textQuestion.answerIds) })
    }
    camp.mapAnswerPackIdByUserId.delete(user._id.toString())
    if (role == 'nong') {
        await camp.updateOne({
            mapAnswerPackIdByUserId: camp.mapAnswerPackIdByUserId,
            nongAnswerPackIds: swop(answerContainer._id, null, camp.nongAnswerPackIds),
        })
        await user.updateOne({ nongAnswerPackIds: swop(answerContainer._id, null, user.nongAnswerPackIds) })
    } else {
        await camp.updateOne({
            mapAnswerPackIdByUserId: camp.mapAnswerPackIdByUserId,
        ...
        await user.updateOne({ peeAnswerPackIds: swop(answerContainer._id, null, user.peeAnswerPackIds) })
    }
